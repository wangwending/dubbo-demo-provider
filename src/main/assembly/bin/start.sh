...
cd `dirname $0`
BIN_DIR=`pwd`
echo "bin file dir: "$BIN_DIR
cd ..
DEPLOY_DIR=`pwd`
echo "deploy file dir: "$DEPLOY_DIR
CONF_DIR=$DEPLOY_DIR/conf
echo "conf file dir: "$CONF_DIR

SERVER_NAME=`sed '/dubbo.provider.name/!d;s/.*=//' conf/dubbo.properties | tr -d '\r'`
SERVER_PROTOCOL=`sed '/dubbo.protocol.name/!d;s/.*=//' conf/dubbo.properties | tr -d '\r'`
SERVER_PORT=`sed '/dubbo.port/!d;s/.*=//' conf/dubbo.properties | tr -d '\r'`
...
while [ $COUNT -lt 1 ]; do    
    ...
    if [ -n "$SERVER_PORT" ]; then
        if [ "$SERVER_PROTOCOL" == "dubbo" ]; then
    	    COUNT=`echo status | nc -i 1 127.0.0.1 $SERVER_PORT | grep -c OK` || { echo "nc command failed"; exit 1; }
        else
